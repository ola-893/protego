...
const examplePromptsTemplate: string[] = [
  "analyze my wallet {WALLET_ADDRESS}",
  "analyze my testnet wallet {WALLET_ADDRESS}",
  "Recommend me staking opportunities",
];
...
export function ExamplePrompts({ onPromptSelect, walletAddress }: ExamplePromptsProps) {
  // Replace placeholder with actual wallet address or fallback text
  const examplePrompts = examplePromptsTemplate.map(prompt => {
    if (prompt.includes("{WALLET_ADDRESS}")) {
      return walletAddress 
        ? prompt.replace("{WALLET_ADDRESS}", walletAddress)
        : prompt.replace("{WALLET_ADDRESS}", "[paste wallet address here]");
    }
    return prompt;
  });
